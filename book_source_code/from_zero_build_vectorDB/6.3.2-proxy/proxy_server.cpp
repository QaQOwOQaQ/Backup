...
: masterServerHost_(masterServerHost), masterServerPort_(masterServerPort), instanceId_(instanceId), curlHandle_(nullptr), activeNodesIndex_(0) , nextNodeIndex_(0), running_(true), activePartitionIndex_(0), activeUserCredentialsIndex_(0) {
    initCurl();
    setupForwarding();
...
void ProxyServer::start(int port) {
    ...
    startUserCredentialsUpdateTimer();

    httpServer_.set_payload_max_length(64 * 1024 * 1024);
...
void ProxyServer::setupForwarding() {
    ...
    // 添加处理获取 JWT 的新路由
    httpServer_.Post("/getJwtToken", [this](const httplib::Request& req, httplib::Response& res) {
        handleJwtTokenRequest(req, res);
    });
    // 在此处根据需要添加更多的转发规则
...
void ProxyServer::forwardRequest(const httplib::Request& req, httplib::Response& res, const std::string& path) {
    ...
    if (authHeader.rfind("Bearer ", 0) == 0) {
        jwtToken = authHeader.substr(7); // 提取 JWT
    }

    std::string username;
    if (jwtToken.empty() || !validateJwt(jwtToken, username)) {
        res.status = 401; // Unauthorized
        res.set_content("Invalid or missing JWT", "text/plain");
        return;
    }
...
httplib::Response ProxyServer::sendRequestToPartition(const httplib::Request& originalReq, const std::string& path, int partitionId) {
    ...
    GlobalLogger->info("Forwarding request to partition node: {} with original method {}", targetUrl, originalReq.method);
    ...
    if (originalReq.method == "POST") {
        ...
        curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE, originalReq.body.size());
...
void ProxyServer::processAndRespondToBroadcast(httplib::Response& res, const std::vector<httplib::Response>& allResponses, uint k) {
    ...
    for (const auto& response : allResponses) {
        GlobalLogger->debug("Processing response from a partition status {} body {}", response.status, response.body);
...
void ProxyServer::forwardToTargetNode(const httplib::Request& req, httplib::Response& res, const std::string& path, const NodeInfo& targetNode) {
    ...
    if (req.method == "POST") {
        GlobalLogger->info("Post request to: {} with body {}", targetUrl, req.body);
         // 设置请求类型为 POST
        curl_easy_setopt(curlHandle_, CURLOPT_POST, 1L);

        // 设置 POST 请求体
        curl_easy_setopt(curlHandle_, CURLOPT_POSTFIELDS, req.body.c_str());
        curl_easy_setopt(curlHandle_, CURLOPT_POSTFIELDSIZE, req.body.size());
    ...
    if (curl_res != CURLE_OK) {
        ...
        GlobalLogger->info("Received response from server response_data {}", response_data);
        // 确保响应数据不为空
...
void ProxyServer::fetchAndUpdateUserCredentials() {
    ...
    int inactiveIndex = activeUserCredentialsIndex_.load() ^ 1; // 获取非活动数组的索引
...
void ProxyServer::startUserCredentialsUpdateTimer() {
    std::thread([this]() {
        while (running_) {
            std::this_thread::sleep_for(std::chrono::minutes(5)); // 每5分钟更新一次
            fetchAndUpdateUserCredentials();
        }
    }).detach();
}
...
bool ProxyServer::generateJwt(const std::string& username, std::string& jwtToken) {
    int activeIndex = activeUserCredentialsIndex_.load(); // 获取当前活动的用户凭证数组索引
    ...
    // 使用用户名和密码生成一个唯一的 JWT 密钥
    std::string jwtSecret = "SecretBase" + it->second.first + it->second.second; // 示例：连接一些基础密钥和用户信息
    ...
    try {
        ...
        auto token = jwt::create()
            ...
            .set_expires_at(std::chrono::system_clock::now() + std::chrono::minutes{30}) // 设置 30 分钟的有效期
            .set_payload_claim("username", jwt::claim(username));

        jwtToken = token.sign(jwt::algorithm::hs256{jwtSecret});
        return true; // 成功生成
    } catch (const std::exception& e) {
        GlobalLogger->error("Failed to generate JWT for {}: {}", username, e.what());
        return false; // 生成 JWT 时出错
    }
}
