...
std::vector<Document> generateWriteData(int numberOfRequests, int dim) {
...
std::vector<Document> generateTestData(int numberOfRequests, int dim, TestType testType) {
    ...
    for (int i = 0; i < numberOfRequests; ++i) {
        ...
        if (testType == TestType::Write || testType == TestType::Both) {
            doc.AddMember("id", i, allocator);
            doc.AddMember("vectors", vectors, allocator);
        }

        if (testType == TestType::Read || testType == TestType::Both) {
            doc.AddMember("vectors", vectors, allocator);
...
std::chrono::duration<double> performOperation(const Document& doc, const char* url) {
...
void executeBenchmark(const std::vector<Document>& testData, int numThreads, TestType testType, const std::string& writeUrl, const std::string& readUrl) {
